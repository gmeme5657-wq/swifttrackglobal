...
create table public.shipments(id uuid primary key default gen_random_uuid(),tracking_number text unique not null,sender_name text not null,sender_address text not null default '',receiver_name text not null,receiver_email text not null default '',receiver_address text not null default '',origin_city text not null,origin_lat double precision not null,origin_lng double precision not null,destination_city text not null,destination_lat double precision not null,destination_lng double precision not null,current_lat double precision not null,current_lng double precision not null,weight_kg numeric(10,2),length_cm numeric(10,2),width_cm numeric(10,2),height_cm numeric(10,2),service_type text not null default 'Standard',status public.shipment_status not null default 'Order Placed',driver_id uuid references public.profiles(id),created_at timestamptz not null default now(),updated_at timestamptz not null default now());
